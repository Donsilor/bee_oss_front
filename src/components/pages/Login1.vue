  ...
  <div class="login-wrap">
    <div class="ms-login">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px">
        <div class="ms-title">星络科技管理平台</div>
        ...
        <div class="login-btn">
          <el-button @click.native="submitForm('ruleForm')" :loading="isBtnLoading">{{btnText}}</el-button>
        </div>
      </el-form>
    </div>
  </div>
...
// import { login, getAuthorityList } from '@/views/UserMgmt/login/apis'
...
export default {
  ...
  methods: {
    ...
    //           sessionStorage.setItem('userUuid', result.data.uuid)
    ...
    //           let userRouters = ''
    //           userRouters = getPermission(this.$store.getters['common/getUserInfo'], userResourcePermission)
    //
    ...
    //           self.$router.push(homePath)
    //         }
    //         getAuthorityList({ uuid: result.data.uuid }).then((res) => {
    //           sessionStorage.authorityList = JSON.stringify({})
    //           sessionStorage.setItem('authorityList', JSON.stringify(res.data))
    //
    //           // linxuekai 2019-10-31 10:32:07
    //           this.$store.commit('global/updatePermissions', res.data)
    //         })
