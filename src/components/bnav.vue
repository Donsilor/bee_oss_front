			...
			<template v-for="item in menuData">
				<template v-if="item.children">
					<el-submenu :index="item.path" :key="item.path">
						<template slot="title">
							<i class="icon-pp" :class="item.icon"></i>{{item.title}}</template>
						<div v-for="subItem in item.children" style="overflow: hidden" :key="subItem.path" v-show="!subItem.needAdmin || (subItem.needAdmin && isAdmin)">
				...
				<template v-if="!item.children">
					<el-menu-item :index="item.path" :key="item.path" :route="{ path: item.path }">
						<i class="icon-pp" :class="item.icon"></i>{{item.title}}</el-menu-item>
				</template>
			</template>
		</el-menu>
	</div>
