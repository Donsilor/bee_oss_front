	...
	<div class="versions-mapping">
		...
		<div class="m-b-20" v-for="item in currenData.device" v-if="inputType === 1 || inputType === 2 || inputType === 4 || inputType === 6">
			...
			<div class="version-list-line">
				...
				<span class="version-list" v-for="inner in item.h5">{{inner}}</span>
			</div>
		</div>
			...
			<div class="m-b-20">
				...
				<div v-for="item in currenData.router">
					...
					<div class="version-list-line">
						...
						<span class="version-list" v-for="inner in item.andorid">{{inner}}</span>
					</div>
					<div class="version-list-line">
						<span class="label-name">android pad: </span>
						<span class="version-list" v-for="inner in item.andorid_pad">{{inner}}</span>
					</div>
					<div class="version-list-line">
						<span class="label-name">ios app: </span>
						<span class="version-list" v-for="inner in item.ios">{{inner}}</span>
					</div>
				</div>
			</div>
			<div class="version-list-line">
				<span class="label-name">适配H5版本号: </span>
				<span class="version-list" v-for="inner in currenData.h5">{{inner}}</span>
			</div>
		</div>
		<div v-for="item in currenData.device" v-if="inputType === 5">
			...
			<div class="version-list-line">
				<span class="label-name">android app: </span>
				<span class="version-list" v-for="inner in item.router.andorid">{{inner}}</span>
			</div>
			<div class="version-list-line">
				<span class="label-name">android pad: </span>
				<span class="version-list" v-for="inner in item.router.andorid_pad">{{inner}}</span>
			</div>
			<div class="version-list-line">
				<span class="label-name">ios app: </span>
				<span class="version-list" v-for="inner in item.router.ios">{{inner}}</span>
			</div>
		</div>
	</div>
</template>
