  ...
  <div class="page-content config-page">
    ...
    <div v-if="!firstTableShow">
      ...
      <div class="p-r" style="height: 40px; padding-top: 10px">
        ...
        <el-button type="text" class="btn-back" @click="backToList">返回上一级</el-button>
        <template v-if="inputType === 3 || inputType === 8">
          <el-tabs v-model="activeName" @tab-click="deviceRouterChange">
            <el-tab-pane label="子设备列表" name="devices"/>
            <el-tab-pane label="子路由列表" name="routers"/>
          </el-tabs>
        </template>
      </div>
      <!--子设备列表-->
      ...
      <div class="page-line">
        <el-pagination small :total="totalItem_two" layout="prev, pager, next" :page-size="10" @current-change="pageChange_two" :current-page.sync="currentPage_two"/>
      </div>
    </div>
    <!--版本匹配搜索-->
    <el-dialog :visible.sync="filterPopoverFlag" title="版本匹配搜索">
      <filter-form key="filterForms" ref="filterForms" @filterVersionsParent="filterVersions" :type="type" @closeFilterBox="filterPopoverFlag = false;" :brand-idoptions="brandIDOptions" :type-idoptions="typeIDOptions" :product-idoptions="productIDOptions" :input-type="inputType" :product="product"
:router="router"/>
    </el-dialog>
    <!--版本详情-->
    <el-dialog :visible.sync="infoBoxFlag" title="版本详情">
      <version_detail ref="versionDetails" :rule-form-detail="ruleFormDetail" :device-obj="currentDetailObj"/>
    ...
    <el-dialog :title="this.addEditFlag?'录入版本':'编辑版本'" :visible.sync="importBoxFlag">
      <version-input key="versionInputs" ref="versionInputs" @importSubmitParent="importSubmit" :brandIDOptions="brandIDOptions" @closeImportBox="importBoxFlag = false;" :type-idoptions="typeIDOptions" :product-idoptions="productIDOptions" :input-type="inputType" :product="product" :type="type"
:os_type="os_type" :add-edit-flag="addEditFlag" :edit-data-obj="editDataObj" :released-flag="releasedFlag" :router-pid-list="routerPidList" :active-name="activeName"/>
    </el-dialog>
    <!--推送升级-->
    <el-dialog :visible.sync="pushBoxFlag" title="推送升级">
      <push-update ref="pushUpdates" @pushUpdateParent="pushUpdate" :type="type" @closePushBox="pushBoxFlag = false;" :input-type="inputType" :push-data-obj="pushDataObj"/>
    </el-dialog>
    <!--操作日志-->
    <el-dialog :visible.sync="operateLogLayer" title="操作日志">
      <operate_log :operate-log-list="operateLogList"/>
      <span slot="footer" class="dialog-footer">
        <el-button style="border:none;" @click="operateLogLayer = false">取 消</el-button>
      </span>
    </el-dialog>
    <!--支持版本-->
    <el-dialog :visible.sync="supportLayer" title="支持版本">
      <version-mapping ref="supportLayer" :input-type="inputType"/>
      <span slot="footer" class="dialog-footer">
        <el-button style="border:none;" @click="supportLayer = false">取 消</el-button>
      </span>
    </el-dialog>
  </div>
</template>
